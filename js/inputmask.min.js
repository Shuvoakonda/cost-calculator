                ...
                if (isValidElement(this[0]))
                    for (var eventRegistry = this[0].eventRegistry, elem = this[0], _events = "string" == typeof events ? events.split(" ") : [events.type], endx = 0; endx < _events.length; endx++) {
                        ...
                        if (void 0 !== document && "global" === namespace) {
                            ...
                            } else evnt = document.createEventObject(), evnt.eventType = ev, evnt.detail = arguments[1], events.type && DependencyLib.extend(evnt, events), elem.fireEvent("on" + evnt.eventType, evnt)
                        } else if (void 0 !== eventRegistry[ev])
                            if (events = events.type ? events : DependencyLib.Event(events), "global" === namespace)
            ...
            function alternate(pos, c, strict, fromIsValid, rAltPos) {
                ...
                function insertPosition(insert) {
                    if (insert && isValidRslt && void 0 !== c) {
                        ...
                        for (i = decisionPos; i < getLastValidPosition() + 1; i++) validPos = getMaskSet().validPositions[i], (void 0 === validPos || 1 == validPos.match.static) && i < pos + posOffset && posOffset++;
            ...
            var EventRuler = {
                ...
                EventHandlers = {
                    ...
                    mouseenterEvent: function mouseenterEvent() {
                        var input = this;
                        mouseEnter = !0, document.activeElement !== this && (this.placeholder !== originalPlaceholder && (originalPlaceholder = this.placeholder), opts.showMaskOnHover && HandleNativePlaceholder(this, (isRTL ? getBuffer().slice().reverse() : getBuffer()).join("")))
                    },
                ...
                if ($.each(inputValue, function (ndx, charCode) {
                    ...
                    if (isComplete(getBuffer()))
                        ...
                        for (; sndx = staticMatches.pop();) {
                            var keypress = new $.Event("_checkval"),
                                nextSndx = sndx + 1;
                            for (validPos = getMaskSet().validPositions[sndx], validPos.generatedInput = !0, keypress.which = validPos.input.charCodeAt(0);
                                (nextValid = getMaskSet().validPositions[nextSndx]) && nextValid.input === validPos.input;) nextSndx++;
